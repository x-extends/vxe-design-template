...
const navList = computed(() => {
  ...
  const rest = XEUtils.findTree(userStore.menuTreeList, item => item.name === routeName, { children: 'children' })
  if (rest) {
    const { nodes } = rest
...
const tabList = computed(() => {
  return userStore.userTabs.map(item => {
    const rest = XEUtils.findTree(userStore.menuTreeList, obj => obj.name === item.name, { children: 'children' })
    return {
      title: rest ? rest.item.title : item.name,
      name: item.name,
      path: item.path,
      routeLink: {
        name: item.routeName,
